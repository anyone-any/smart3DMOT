...
def MMDF(detections_3D_camera, detection_2D, detection_3Dto2D, additional_info):
    '''
    ...
    iou_threshold = 0.4
    iou_matrix = np.zeros(
        (len(detection_2D), len(detection_3Dto2D)), dtype=np.float32)
    for d1, det_2D in enumerate(detection_2D):
        for d2, det_3Dto2D in enumerate(additional_info):
            iou_matrix[d1, d2] = iou_batch(
                det_2D, det_3Dto2D[2:6])  # det: 8 x 3, trk: 8 x 3
    if min(iou_matrix.shape) > 0:
        a = (iou_matrix > iou_threshold).astype(np.int32)
        if a.sum(1).max() == 1 and a.sum(0).max() == 1:
            matched_indices = np.stack(np.where(a), axis=1)
        else:
            matched_indices = linear_assignment(-iou_matrix, -iou_threshold)
    else:
    ...
    for detection_2D_idx, detection_3Dto2D_idx in matched:
        ...
        detection_3Dto2D_fusion.append(
            detection_3Dto2D[detection_3Dto2D_idx].tolist())
        detection_3D_fusion.append(
            detections_3D_camera[detection_3Dto2D_idx].tolist())

    for unmatched_detections_2D_idx in unmatched_detections_2D:
        detection_2D_only.append(
            detection_2D[unmatched_detections_2D_idx].tolist())

    for unmatched_detections_2Dto3D_idx in unmatched_detection_3Dto2D:
        detection_2Dto3D_only.append(
            detection_3Dto2D[unmatched_detections_2Dto3D_idx].tolist())
        detection_3D_only.append(
            detections_3D_camera[unmatched_detections_2Dto3D_idx].tolist())
        detection_3D_only_info.append(
            additional_info[unmatched_detections_2Dto3D_idx])

    detection_3D_fusion = {'dets_3d_fusion': detection_3D_fusion,
                           'dets_3d_fusion_info': detection_3D_fusion_info}
    detection_3D_only = {'dets_3d_only': detection_3D_only,
    ...
    return np.array(detection_2D_fusion), np.array(detection_3Dto2D_fusion), detection_3D_fusion, \
        np.array(detection_2D_only), np.array(
            detection_2Dto3D_only), detection_3D_only
